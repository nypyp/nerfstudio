...
@dataclass
class MySemanticNerfModelConfig(NerfactoModelConfig):
    """Nerfacto Model Config"""
    
    _target: Type = field(default_factory=lambda: MySemanticNerfModel)
...
class MySemanticNerfModel(Model):
    """MySemanticNerf Model
    
    Args:
        config Nerfacto configuration to instantiate model
    """
    
    config: MySemanticNerfModelConfig
    
    def __init__(self, config: MySemanticNerfModelConfig, metadata: Dict, **kwargs) -> None:
    ...
    def populate_modules(self):
        ...
        # metrics for semantics
        if self.config.use_semantics:
            from torchmetrics import JaccardIndex
            self.miou  = JaccardIndex(num_classes=len(self.semantics.classes), task='multiclass')
    ...
    def get_image_metrics_and_images(
        ...
        if self.config.use_semantics:
            predicted_semantics = torch.argmax(torch.nn.functional.softmax(outputs["semantics"], dim=-1), dim=-1)
            gt_semantics = batch["semantics"][..., 0].long().to(self.device)
            
            predicted_semantics = predicted_semantics.long().flatten()
            gt_semantics = gt_semantics.long().flatten()
            
            miou = self.miou(predicted_semantics, gt_semantics)
